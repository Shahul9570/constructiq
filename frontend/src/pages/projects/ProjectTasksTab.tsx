...
export function ProjectTasksTab({ projectId }: ProjectTasksTabProps) {
  ...
  const [formData, setFormData] = useState<{ name: string, description: string, weight_percentage: number, assigned_to_id?: string }>({ 
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setIsOpen(false)
      setFormData({ name: '', description: '', weight_percentage: 0, assigned_to_id: 'none' })
