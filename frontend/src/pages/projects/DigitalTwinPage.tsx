...
export default function DigitalTwinPage() {
  ...
  // Client view: no model upload prompt — show a clean message
  if (!data?.model_url) {
    return (
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Replace 3D Model</h1>
        </div>
        <div className="flex flex-col items-center justify-center h-[60vh] bg-slate-900 rounded-xl border border-slate-800 space-y-4">
          <input
            type="file"
          ...
          <h2 className="text-xl font-medium text-slate-300">Replace 3D Model</h2>
          <div className="flex gap-2">
            ...
            <Button variant="ghost" onClick={() => setForceUpload(false)} className="text-slate-500">
              Cancel
            </Button>
          </div>
        </div>
    ...
    <div className="space-y-4 h-[calc(100vh-6rem)] flex flex-col">
      <div className="flex items-center gap-4 shrink-0">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-3">
            ...
            {isClient && (
              <Badge className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5 text-xs font-normal">
          ...
          <p className="text-sm text-slate-500">
            {isClient ? 'Real-time construction progress — read-only client view' : 'Interactive 3D structural model'}
          </p>
        </div>
      ...
      <div className="flex-1 flex gap-4 overflow-hidden">
        ...
        <div className="flex-1 relative rounded-xl overflow-hidden border border-slate-800 shadow-2xl">
          ...
          {/* Progress overlay — clients see a read-only info panel */}
          <ProgressOverlay
            projectId={projectId}
            selectedMeshId={selectedMeshId}
          ...
          {isManager && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4 z-10">
              ...
              >
                ...
                <Input
                  placeholder="e.g. 'Mark all interior walls as 50% complete'"
